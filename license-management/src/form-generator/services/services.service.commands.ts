...
import { TaskRepository } from 'src/Task/task/persistance/task.repository ';
...
export class ServicesServiceCommands {
    ...
    async createTask(command: CreateTaskCommand): Promise<any> {
        // try {
        //     const formEntity = CreateTaskCommand.fromCommands(command)
        //     this.formDomain = await this.taskRepository.save(formEntity)
        //     if (!this.formDomain) {
        //         throw new NotFoundException(`Failed to create Form`);
        //     }
        //     return FormResponse.fromDomain(this.formDomain)
